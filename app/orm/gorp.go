...
var versFieldConst = "[orm_ver_field]"
...
// SqlExecutor exposes orm operations that can be run from Pre/Post
...
// DynamicTable allows the users of orm to dynamically
...
func toSliceType(i interface{}) (reflect.Type, error) {
	...
	if t.Kind() != reflect.Ptr {
		...
		if t.Kind() == reflect.Slice {
			return nil, fmt.Errorf("orm: cannot SELECT into a non-pointer slice: %v", t)
		}
		return nil, nil
	}
...
func toType(i interface{}) (reflect.Type, error) {
	...
	if t.Kind() != reflect.Struct {
		return nil, fmt.Errorf("orm: cannot SELECT into this type: %v", reflect.TypeOf(i))
	}
	return t, nil
}
		...
		if bi.autoIncrIdx > -1 {
			...
			case IntegerAutoIncrInserter:
				...
				if (k == reflect.Int) || (k == reflect.Int16) || (k == reflect.Int32) || (k == reflect.Int64) {
					...
					return fmt.Errorf("orm: cannot set autoincrement value on non-Int field. SQL=%s  autoIncrIdx=%d autoIncrFieldName=%s", bi.query, bi.autoIncrIdx, bi.autoIncrFieldName)
				}
			...
			case TargetQueryInserter:
				...
				if idQuery == "" {
					return fmt.Errorf("orm: cannot set %s value if its ColumnMap.GeneratedIdQuery is empty", bi.autoIncrFieldName)
				}
			...
			default:
				return fmt.Errorf("orm: cannot use autoincrement fields on dialects that do not implement an autoincrementing interface")
			}
		} else {
