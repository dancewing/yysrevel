...
// DbMap is the root orm mapping object. Create one of these for each
// database schema you wish to map.  Each DbMap contains a list of
// mapped tables.
//
// Example:
//
//     dialect := orm.MySQLDialect{"InnoDB", "UTF8"}
//     dbmap := &orm.DbMap{Db: db, Dialect: dialect}
//
type DbMap struct {
...
// AddTable registers the given interface type with orm. The table name
...
// AddTableDynamic registers the given interface type with orm.
...
func (m *DbMap) TableFor(t reflect.Type, checkPK bool) (*TableMap, error) {
	...
	if checkPK && len(table.keys) < 1 {
		e := fmt.Sprintf("orm: no keys defined for table: %s",
			table.TableName)
...
func (m *DbMap) DynamicTableFor(tableName string, checkPK bool) (*TableMap, error) {
	...
	if !found {
		return nil, fmt.Errorf("orm: no table found for name: %v", tableName)
	}
...
func (m *DbMap) tableForPointer(ptr interface{}, checkPK bool) (*TableMap, reflect.Value, error) {
	...
	if ptrv.Kind() != reflect.Ptr {
		e := fmt.Sprintf("orm: passed non-pointer: %v (kind=%v)", ptr,
			ptrv.Kind())
