...
func SelectOne(m *DbMap, e SqlExecutor, holder interface{}, query string, args ...interface{}) error {
	...
	if t.Kind() == reflect.Ptr {
		...
		return fmt.Errorf("orm: SelectOne holder must be a pointer, but got: %t", holder)
	}
	...
	if t.Kind() == reflect.Struct {
		...
		if list != nil && len(list) > 0 { // FIXME: invert if/else
			...
			if len(list) > 1 {
				return fmt.Errorf("orm: multiple rows returned for: %s - %v", query, args)
			}
...
func rawselect(m *DbMap, exec SqlExecutor, i interface{}, query string,
	...
	if !intoStruct && len(cols) > 1 {
		return nil, fmt.Errorf("orm: select into non-struct slice requires 1 column, got %d", len(cols))
	}
