...
// https://github.com/dancewing/yysrevel/orm
...
// See: https://github.com/dancewing/yysrevel/orm/issues/175
...
func (p *PersonValuerScanner) Scan(value interface{}) (err error) {
	...
	case []byte:
		...
		// orm to migrate to other database engines.
...
func (me testTypeConverter) FromDb(target interface{}) (orm.CustomScanner, bool) {
	switch target.(type) {
	case *Person:
		...
		return orm.CustomScanner{new(string), target, binder}, true
	case *CustomStringType:
		binder := func(holder, target interface{}) error {
			s, ok := holder.(*string)
			if !ok {
	...
	case *CustomDate:
		...
		return orm.CustomScanner{new(time.Time), target, binder}, true
	}

	return orm.CustomScanner{}, false
}

func (p *Person) PreInsert(s orm.SqlExecutor) error {
...
func (p *Person) PostInsert(s orm.SqlExecutor) error {
	p.LName = "postinsert"
	return nil
}

func (p *Person) PreUpdate(s orm.SqlExecutor) error {
	p.FName = "preupdate"
	return nil
}

func (p *Person) PostUpdate(s orm.SqlExecutor) error {
	p.LName = "postupdate"
	return nil
}

func (p *Person) PreDelete(s orm.SqlExecutor) error {
	p.FName = "predelete"
	return nil
}

func (p *Person) PostDelete(s orm.SqlExecutor) error {
	p.LName = "postdelete"
	return nil
}

func (p *Person) PostGet(s orm.SqlExecutor) error {
	p.LName = "postget"
	return nil
}
...
func dynamicTablesTest(t *testing.T, dbmap *orm.DbMap) {
...
func dynamicTablesTestGetUpdateGet(t *testing.T,
	...
	// that the common orm APIs are working well with dynamic table
...
func TestOptimisticLocking(t *testing.T) {
	...
	if _, ok := err.(orm.OptimisticLockError); !ok {
		t.Errorf("update - Expected orm.OptimisticLockError, got: %v", err)
	}
	if count != -1 {
	...
	if _, ok := err.(orm.OptimisticLockError); !ok {
		t.Errorf("delete - Expected orm.OptimisticLockError, got: %v", err)
	}
	if count != -1 {
...
func testCrudInternal(t *testing.T, dbmap *orm.DbMap, val testable) {
...
func TestColumnFilter(t *testing.T) {
	...
	inv1.Memo = "c"
	inv1.IsPaid = true
	_updateColumns(dbmap, func(col *orm.ColumnMap) bool {
...
// TestSqlExecutorInterfaceSelects ensures that all orm.DbMap methods starting with Select...
// are also exposed in the orm.SqlExecutor interface. Select...  functions can always
// run on Pre/Post hooks.
func TestSqlExecutorInterfaceSelects(t *testing.T) {
	dbMapType := reflect.TypeOf(&orm.DbMap{})
	sqlExecutorType := reflect.TypeOf((*orm.SqlExecutor)(nil)).Elem()
	...
	for i := 0; i < numDbMapMethods; i += 1 {
		...
		if _, found := sqlExecutorType.MethodByName(dbMapMethod.Name); !found {
			t.Errorf("Method %s is defined on orm.DbMap but not implemented in orm.SqlExecutor",
				dbMapMethod.Name)
		}
	}
}
...
func TestNullTime(t *testing.T) {
	...
	if ent.Time.Valid {
		t.Error("orm.NullTime returns valid but expected null.")
	}
	...
	if !ent.Time.Valid {
		t.Error("orm.NullTime returns invalid but expected valid.")
	}
...
// See: https://github.com/dancewing/yysrevel/orm/issues/86
...
func TestQuoteTableNames(t *testing.T) {
	...
	dbmap.TraceOn("", log.New(logBuffer, "ormtest:", log.Lmicroseconds))
...
func TestMysqlPanicIfDialectNotInitialized(t *testing.T) {
	...
	defer func() {
		r := recover()
		if r == nil {
			t.Error("db.CreateTables() should panic if db is initialized with an incorrect orm.MySQLDialect")
		}
	}()

	// invalid MySQLDialect : does not contain Engine or Encoding specification
	dialect := orm.MySQLDialect{}
	db := &orm.DbMap{Db: connect(driver), Dialect: dialect}
...
func newDbMap() *orm.DbMap {
	dialect, driver := dialectAndDriver()
	dbmap := &orm.DbMap{Db: connect(driver), Dialect: dialect}
	if debug {
		dbmap.TraceOn("", log.New(os.Stdout, "ormtest: ", log.Lmicroseconds))
	}
	return dbmap
}

func dropAndClose(dbmap *orm.DbMap) {
...
func dialectAndDriver() (orm.Dialect, string) {
	switch os.Getenv("GORP_TEST_DIALECT") {
	case "mysql":
		return orm.MySQLDialect{"InnoDB", "UTF8"}, "mymysql"
	case "gomysql":
		return orm.MySQLDialect{"InnoDB", "UTF8"}, "mysql"
	case "postgres":
		return orm.PostgresDialect{}, "postgres"
	case "sqlite":
		return orm.SqliteDialect{}, "sqlite3"
	}
	panic("GORP_TEST_DIALECT env variable is not set or is invalid. Please see README.md")
}

func _insert(dbmap *orm.DbMap, list ...interface{}) {
...
func _update(dbmap *orm.DbMap, list ...interface{}) int64 {
...
func _updateColumns(dbmap *orm.DbMap, filter orm.ColumnFilter, list ...interface{}) int64 {
...
func _del(dbmap *orm.DbMap, list ...interface{}) int64 {
...
func _get(dbmap *orm.DbMap, i interface{}, keys ...interface{}) interface{} {
...
func selectInt(dbmap *orm.DbMap, query string, args ...interface{}) int64 {
	i64, err := orm.SelectInt(dbmap, query, args...)
	if err != nil {
		panic(err)
	}
...
func selectNullInt(dbmap *orm.DbMap, query string, args ...interface{}) sql.NullInt64 {
	i64, err := orm.SelectNullInt(dbmap, query, args...)
	if err != nil {
		panic(err)
	}
...
func selectFloat(dbmap *orm.DbMap, query string, args ...interface{}) float64 {
	f64, err := orm.SelectFloat(dbmap, query, args...)
	if err != nil {
		panic(err)
	}
...
func selectNullFloat(dbmap *orm.DbMap, query string, args ...interface{}) sql.NullFloat64 {
	f64, err := orm.SelectNullFloat(dbmap, query, args...)
	if err != nil {
		panic(err)
	}
...
func selectStr(dbmap *orm.DbMap, query string, args ...interface{}) string {
	s, err := orm.SelectStr(dbmap, query, args...)
	if err != nil {
		panic(err)
	}
...
func selectNullStr(dbmap *orm.DbMap, query string, args ...interface{}) sql.NullString {
	s, err := orm.SelectNullStr(dbmap, query, args...)
	if err != nil {
		panic(err)
	}
...
func rawExec(dbmap *orm.DbMap, query string, args ...interface{}) sql.Result {
...
func rawSelect(dbmap *orm.DbMap, i interface{}, query string, args ...interface{}) []interface{} {
...
func tableName(dbmap *orm.DbMap, i interface{}) string {
	t := reflect.TypeOf(i)
...
func columnName(dbmap *orm.DbMap, i interface{}, fieldName string) string {
	t := reflect.TypeOf(i)
	if table, err := dbmap.TableFor(t, false); table != nil && err == nil {
