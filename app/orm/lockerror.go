...
func (e OptimisticLockError) Error() string {
	if e.RowExists {
		return fmt.Sprintf("orm: OptimisticLockError table=%s keys=%v out of date version=%d", e.TableName, e.Keys, e.LocalVersion)
	}

	return fmt.Sprintf("orm: OptimisticLockError no row found for table=%s keys=%v", e.TableName, e.Keys)
}
